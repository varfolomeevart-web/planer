...
const checks: [string, boolean][] = [
  ...
  ['материал перегородки 1 в ТЗ', brief.includes('конструкция: ГКЛ на металлокаркасе, покраска')],
  ['без материала — без слова «конструкция»', !brief.split('Перегородка 2:')[1].split('\n')[0].includes('конструкция')],
  ['легенда системы координат', brief.includes('Система координат: сантиметры')],
  ['строка о прилагаемой 3D-модели GLB и маппинге осей', brief.includes('координатная 3D-модель GLB') && brief.includes('Y — высота/100')],
]
let ok = true
...
const rtChecks: [string, boolean][] = [
  ...
  ['round-trip: материал сохранён', rp[0]?.material === 'ГКЛ на металлокаркасе, покраска'],
